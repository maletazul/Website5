...
if (isset($_POST['submit_message'])) {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL) || strlen($email) > 50 ) {
        ...
        $response['error']['email'] = "Um email valido e necessario";
    }
    if (empty($name) ) {
        http_response_code(403);
        $response['error']['name'] = 'Nome é obrigatorio ';
    }
    if (empty($message)) {
        http_response_code(403);
        $response['error']['message'] = 'A mensagem tem de ter conteudo';
    }
    ...
    if (!isset($response['error']) || $response['error'] === '') {
        ...
        $subject = "Contato via website de ".$name;
        
        // Build the email content.
        $email_content = $message."\n \n";
        $email_content .= "Obrigado,";
        ...
        if ( mail($recipient, $subject, $email_content, $email_headers) ) {
            ...
            $response['error'] = 'Oops! Algo correu mal, nao foi possivel \'t enviar a sua mensagem';
            $content = 'Erro de envio - não foi possivel enviar a mensagem'. "\r\n" . $content;
...
if (isset($_POST['submit_email'])) {
    ...
    if (!isset($response['error']) || $response['error'] === '') {
        ...
        if (mail($recipient, $subject, $email_content, $email_headers)) {
            // Set a 200 (okay) response code.
            http_response_code(200);
            $response['success'] = "Obrigado! Sera contatado em breve.";
            
        } else {
            // Set a 500 (internal server error) response code.
            //http_response_code(500);
            http_response_code(500);
            $response['error'] = "Oops! Algo correu mal, nao foi possivel enviar a sua mensagem.";
